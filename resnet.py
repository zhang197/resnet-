...
class ResNetModel(object):
    ...
    def __init__(self, source, device_str, depth=50, is_training= tf.constant(True, dtype=tf.bool)):#, num_classes=1000
    ...
    def inference(self):
        with tf.device(self.device_str):
            ...
            self.output = tf.expand_dims(avg_pool, 1, name='cnn_output')
    ...
    def load_original_weights(self, session):
        weights_path = '../BaseModel/ResNet-L{}.npy'.format(self.depth)
...
Helper methods
"""
def _get_variable(name, shape, weight_decay=0.0, dtype='float', trainable=True):#(name, shape, initializer, weight_decay=0.0, dtype='float', trainable=True):
    ...
    return tf.get_variable(name, shape=shape, dtype=dtype, regularizer=regularizer,
                           trainable=trainable)
...
def bn(x, is_training):
    ...
    beta = _get_variable('beta', params_shape, trainable=True)#, initializer=tf.zeros_initializer())
    gamma = _get_variable('gamma', params_shape, trainable=True)#, initializer=tf.ones_initializer())

    moving_mean = _get_variable('moving_mean', params_shape, trainable=False) #initializer=tf.zeros_initializer(),
    moving_variance = _get_variable('moving_variance', params_shape, trainable=False)#initializer=tf.ones_initializer(), 
